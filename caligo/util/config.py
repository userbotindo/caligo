...
class BotConfig:
    ...
    def __init__(self) -> "BotConfig":
        ...
        # Optional
        path = self._replace(os.environ.get("DOWNLOAD_PATH"))
        self.downloadPath = Path(path) if path else Path.home() / "downloads"

        self.token = self._replace(os.environ.get("BOT_TOKEN"))
        ...
        self.gdrive_folder_id = self._replace(
            os.environ.get("G_DRIVE_FOLDER_ID"))
        self.gdrive_index_link = self._replace(
        ...
        # Github
        self.github_repo = self._replace(os.environ.get("GITHUB_REPO")
                                         ) or "adekmaulana/caligo"
        self.github_token = self._replace(os.environ.get("GITHUB_TOKEN"))

        # Heroku
        self.heroku_app_name = self._replace(os.environ.get("HEROKU_APP"))
        self.heroku_api_key = self._replace(os.environ.get("HEROKU_API_KEY"))
